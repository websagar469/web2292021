...
// 				$more   = $excerpt_more_link ? '<br><hr> <a class="excerpt-more" href="' . get_permalink() . '">' . $more . '<i class="fas fa-arrow-right"></i></a>' : ' <span class="excerpt-more">' . $more . '</span>';
		...
		if ( $category_display && is_object_in_taxonomy( get_post_type(), $category_display ) ) {
			...
			if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
				foreach ( $terms as $term ) {
				$term_output[] = '<a href="">' . $term->name . '</a>';
				}
				$category_display_text = '<br><br> <span class="category-display"><span class="category-display-label">' . $category_label . '</span> ' . implode( '', $term_output ) . '</span>';
			}
