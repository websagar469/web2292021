...
const Thumb = (props, state) => <div {...props}>{state.valueNow === labels.length ? '∞' : state.valueNow}</div>;
...
export default class Simple extends React.Component {
    constructor(props) {
        ...
        if (this.state.delay < 0) {
            this.state.delay = labels.length;
        }
            ...
            <ul>
                <li>
                    <span className="enabled">
                        ...
                        <input type="hidden" name={this.props.prefix + '[delay]'} value={this.state.delay === labels.length ? -1 : this.state.delay}></input>
